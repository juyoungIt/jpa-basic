...
@SpringBootTest
@EntityScan(basePackages = {"com.howard.jpabasic.section9.section9_4.no_equals"})
public class Section9Test {
    ...
    public void equivalenceComparisonTest() {
        try {
            ...
            System.out.println("memberA.equals(memberB) : " + (memberA.equals(memberB))); // 같은 인스턴스, 같은 값이므로 True
            System.out.println("memberA.equals(memberC) : " + (memberA.equals(memberC))); // True가 출력되기를 기대하지만 그렇지 않다.
            /* 동등성을 비교하는 코드, equals() 를 사용하여 비교하는 것과 같다 */
            assertThat(memberA).isEqualTo(memberB);
            // equals() 를 별도 정의하지 않아 '==' 를 사용하는 기본 equals() 를 사용하여 그렇다.
            assertThat(memberA).isNotEqualTo(memberC);
