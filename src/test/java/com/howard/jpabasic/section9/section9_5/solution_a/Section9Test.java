...
@SpringBootTest
@EntityScan(basePackages = {"com.howard.jpabasic.section9.section9_5.solution_a"})
public class Section9Test {
    ...
    @Test
    @DisplayName("값 타입 컬렉션 - 값 타입 컬렉션을 일대다 단방향 연관관계로 풀어 Entity 승격을 통해 문제를 해결한다.")
    public void valueTypeCollectionToOneToManyRelationTest() {
        try {
            etx.begin();
            ...
            member.getAddressHistory().add(historyA);
            member.getAddressHistory().add(historyB);
            em.persist(member);
